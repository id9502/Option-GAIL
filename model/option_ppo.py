    ...
    def step(self, sample_sar, lr_mult=1.):
        ...
        for _ in range(10):
            ...
            for ind_b in inds.split(self.mini_bs):
                ...
                vf_loss = torch.max((vpred - return_b).square(), (vpred_clip - return_b).square()).mean()
...
class OptionPPO(torch.nn.Module):
    def __init__(self, config: Config , policy: OptionPolicy):
        ...
        self.critic_lo = OptionCritic(config, dim_s=self.policy.dim_s, dim_c=self.policy.dim_c)
    ...
    def _calc_adv(self, sample_scar, train_policy=True, train_option=True):
        with torch.no_grad():
            ...
            for s, c, a, r in sample_scar:
                ...
                vl = vc.gather(dim=-1, index=c[1:]).detach() if train_policy else torch.zeros_like(r)
        ...
        for _ in range(10):
            ...
            for ind_b in inds.split(self.mini_bs):
                ...
                if train_option:
                    ...
                    vpred = (self.critic_lo.get_value(s_b) * fixed_pc_b).sum(dim=-1, keepdim=True)
    ...
    def step(self, sample_sar, lr_mult=1.):
        ...
        for _ in range(10):
            ...
            for ind_b in inds.split(self.mini_bs):
                ...
                pg_loss = -torch.min(advantages_b * ratio, advantages_b * ratio.clamp(1.0 - self.clip_eps, 1.0 + self.clip_eps)).mean(dim=-2).sum()
                loss = pg_loss + vf_loss * 0.5 - self.lambda_entropy * entropy.mean()
                optim.zero_grad()
                loss.backward()
                # after many experiments i find that do not clamp performs the best
