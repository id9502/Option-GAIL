...
from .option_discriminator import OptionDiscriminator, Discriminator, MoEDiscriminator
...
class GAIL(torch.nn.Module):
    def __init__(self, config: Config, dim_s=2, dim_a=2):
        ...
        self.optim = torch.optim.Adam(self.discriminator.parameters(), lr=lr, weight_decay=1.e-3)
...
class MoEGAIL(torch.nn.Module):
    def __init__(self, config: Config, dim_s=2, dim_a=2):
        ...
        self.optim = torch.optim.Adam(list(self.discriminator.parameters()) + self.policy.get_param(low_policy=False), weight_decay=1.e-3)
        self.to(self.device)
    ...
    def step_original_gan(self, sample_sar, demo_sar, n_step=10):
        sp = torch.cat([s for s, a, r in sample_sar], dim=0)
        se = torch.cat([s for s, a, r in demo_sar], dim=0)
        tp = torch.ones(self.mini_bs, self.dim_c, dtype=torch.float32, device=self.device)
        te = torch.zeros(self.mini_bs, self.dim_c, dtype=torch.float32, device=self.device)
        ...
        for _ in range(n_step):
            ...
            for ind_p in inds.split(self.mini_bs):
                ...
                for _ in range(3):
                    ...
                    src = self.discriminator.get_unnormed_d(s_array).squeeze(dim=-1)  # N x C
                    loss = (F.binary_cross_entropy_with_logits(src, t_array, reduction='none') * w_array).mean(dim=-2).sum()
                    lb = (w_array.mean(dim=0) - 0.5).square().sum()
                    le = (w_array.mean(dim=1) - 0.5).square().mean()
                    lv = -((w_array - w_array.mean(dim=0, keepdim=True)).mean(dim=0)).sum()
